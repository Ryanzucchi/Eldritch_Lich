...
export function scheduleResearchReview(note: ResearchNote, quality: number, now = new Date()): Pick<ResearchNote, 'repetition' | 'intervalDays' | 'easinessFactor' | 'nextReviewDate'> {
  ...
  const next = new Date(now); next.setDate(next.getDate() + intervalDays);

export interface DetectedLanguage {
  ...
  confidence: number; // 0.0 a 1.0 (UC-015)
}

export interface DetectedEntity {
  id: string;
  name: string;
  type: 'PERSONAGEM' | 'LOCAL' | 'OBJETO' | 'ORGANIZACAO'; // UC-018
  startIndex: number;
  endIndex: number;
  confidence: number;
}
...
export function detectTextLanguage(text: string): DetectedLanguage {
  ...
  const langPatterns = [
    { code: 'pt' as const, words: [' o ', ' a ', ' do ', ' da ', ' que ', ' para ', ' com ', ' em '] },
    { code: 'en' as const, words: [' the ', ' of ', ' and ', ' to ', ' in ', ' is ', ' you ', ' that '] },
    { code: 'es' as const, words: [' el ', ' la ', ' de ', ' que ', ' en ', ' un ', ' con ', ' para '] },
    { code: 'fr' as const, words: [' le ', ' la ', ' de ', ' et ', ' un ', ' en ', ' que ', ' dans '] },
    { code: 'de' as const, words: [' der ', ' die ', ' das ', ' und ', ' ist ', ' in ', ' mit ', ' zu '] },
    { code: 'it' as const, words: [' il ', ' la ', ' di ', ' e ', ' in ', ' un ', ' con ', ' per '] }
  ];

  let bestLang: 'pt' | 'en' | 'es' | 'fr' | 'de' | 'it' = 'pt';
  let maxScore = 0;
 ...
 */
export function tokenizeAndValidateWords(
  text: string,
  dictionary: string[]
): { word: string; isRecognized: boolean; startIndex: number; endIndex: number }[] {
  const wordsRegex = /\b[\wÀ-ÿ'-]+\b/g;
  let match;
  const results: { word: string; isRecognized: boolean; startIndex: number; endIndex: number }[] = [];
 ...
 */
export function segmentSentences(text: string): { sentence: string; startIndex: number; endIndex: number }[] {
  const sentenceRegex = /[^.!?]+[.!?]+(\s|$)/g;
  let match;
  const sentences: { sentence: string; startIndex: number; endIndex: number }[] = [];
...
export function extractNamedEntities(
  text: string,
  knownEntities: { name: string; type: DetectedEntity['type'] }[] = []
): DetectedEntity[] {
 ...
 */
export function suggestSubfolderGrouping(
  files: { id: string; title: string; content: string }[]
): SubfolderSuggestion[] {

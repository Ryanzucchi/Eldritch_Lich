...
export const AMBIENT_LIBRARY: TrackItem[] = [
  {
    id: 'tr-1',
 ...
 */
export function generateChapterPlaylist(
  manuscriptId: string,
  htmlOrText: string
): PlaylistResult {
  ...
  (Object.keys(keywords) as MoodType[]).forEach(mood => {
  ...
  let dominantMood: MoodType = 'misterio';
  let maxScore = -1;

  (Object.keys(moodScores) as MoodType[]).forEach(mood => {
  ...
  const primaryTrack = AMBIENT_LIBRARY.find(t => t.mood === dominantMood) || AMBIENT_LIBRARY[1];
  const otherTracks = AMBIENT_LIBRARY.filter(t => t.id !== primaryTrack.id);

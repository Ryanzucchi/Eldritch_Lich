 ...
 */
export function processFootnotes(html: string): {
  ...
  const processedHtml = html.replace(
    /<span[^>]*class="[^"]*footnote-box[^"]*"[^>]*data-footnote-text="([^"]+)"[^>]*>([\s\S]*?)<\/span>|<span[^>]*class="[^"]*footnote-box[^"]*"[^>]*title="([^"]+)"[^>]*>([\s\S]*?)<\/span>/gi,
    (match, textAttr1, content1, textAttr2, content2) => {
      count++;
...
export function validateCrossReferences(
  ...
  const validatedHtml = html.replace(
    /<a[^>]*class="[^"]*cross-ref-link[^"]*"[^>]*data-target-id="([^"]+)"[^>]*data-target-title="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi,
    (match, targetId, targetTitle, innerText) => {

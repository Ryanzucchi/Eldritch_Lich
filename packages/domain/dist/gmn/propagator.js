...
exports.propagateStatus = propagateStatus;
/**
 ...
 */
function propagateStatus(nodes, edges) {
    ...
    for (const edge of edges) {
        ...
        if (parents.has(edge.toId))
            parents.get(edge.toId).push(edge.fromId);
    }
        ...
        if (hasUnresolvedParent) {
            if (node.status === 'CONCLUIDO') {
                ...
                const conflictingParents = nodeParents
                    ...
                    .map(p => p.title);
                alerts.push(`A alteração em [${conflictingParents.join(', ')}] impossibilitou a meta "${node.title}". Ajuste o texto ou atualize o grafo.`);
            }
        }
        else {

...
const terms = (text) => (text.toLocaleLowerCase('pt-BR').match(/[\p{L}\p{N}]{3,}/gu) || []).filter(term => !stopwords.has(term));
/** Extrai termos relevantes com TF-IDF local; a coleção inteira compõe o corpus. */
function extractTfIdfKeywords(documents, documentId, limit = 10) {

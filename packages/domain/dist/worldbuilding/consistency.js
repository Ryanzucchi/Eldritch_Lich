...
exports.findNearDuplicateNames = findNearDuplicateNames;
exports.verifyCharacterAgeConsistency = verifyCharacterAgeConsistency;
function findNearDuplicateNames(entities, maximumDistance = 1) {
    const distance = (left, right) => {
        const a = left.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
        const b = right.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
    ...
    for (let left = 0; left < entities.length; left++)
        for (let right = left + 1; right < entities.length; right++) {
            const value = distance(entities[left].name, entities[right].name);
            if (value <= maximumDistance)
                output.push({ primaryId: entities[left].id, duplicateId: entities[right].id, distance: value });
        }
    return output;
}
function verifyCharacterAgeConsistency(characters, familyRelations, currentStoryYear = new Date().getFullYear()) {
    const issues = [];

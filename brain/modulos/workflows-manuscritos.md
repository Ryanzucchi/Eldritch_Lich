...
16. **Extração local sob revisão**: ao colar texto com cabeçalhos de capítulo, ou ao acionar “Analisar N manuscritos”, `manuscript-extraction.ts` identifica capítulos, nomes próprios, datas e parentescos declarados em toda a biblioteca ativa. A interface mostra a revisão em lote com o capítulo de origem de cada achado; somente “Aplicar ao projeto” cria os capítulos separados e grava personagens, entidades, eventos e relações no IndexedDB. Títulos já existentes não são duplicados e o capítulo aberto não muda durante a atualização da lista. Cada detecção é gravada em `automationHistories` com o `manuscriptId` de origem, e a confirmação impede que um nome incidental ou uma inferência fraca altere o universo.
17. **Consulta de manuscritos:** `manuscripts` não possui índice Dexie por `projectId`; os consumidores leem a lista local e filtram pelo projeto ativo. Consultas indexadas por esse campo lançam `SchemaError` e não devem ser introduzidas sem uma migração de schema.
18. **Biblioteca organizada:** o explorer filtra obrigatoriamente pelo projeto ativo, oferece busca por título, filtro por estado e ordenação por estrutura, atualização ou título. A ordenação estrutural reconhece títulos no formato `Livro — Capítulo` (inclusive algarismos romanos), agrupando-os por livro; prefácio e prólogo ficam em “Material inicial”. Cada grupo pode ser recolhido sem alterar os dados do manuscrito.
19. **Exclusão segura de capítulo:** o cabeçalho do documento aberto contém “Excluir”. Após confirmação explícita, o registro recebe `inTrash` e `deletedAt`, desaparece imediatamente da biblioteca e é sincronizado como lixeira; assim a exclusão não destrói o texto de forma irrecuperável.

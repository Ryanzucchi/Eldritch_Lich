...
16. **Extração local conservadora e revisável**: antes de analisar, `manuscript-extraction.ts` decodifica a marcação de importação e normaliza apenas divisões inequívocas de letras vindas de PDF. Conteúdo com HTML escapado continua bloqueado até o autor salvar a normalização. A heurística aceita candidatos de nome somente com recorrência ou forma forte, classifica contexto em personagem/local/item/organização/criatura e exige contexto temporal explícito para eventos. Cada candidato inclui confiança, trecho, capítulo, versão da heurística e `fingerprint` idempotente.
17. **Separação entre sugestão e cânone**: a análise grava o candidato em `extractionCandidates` com estado `PENDING`; não cria ficha, evento, cronologia nem relação. A revisão mostra tipo, confiança e evidência com seleção individual. Somente os itens marcados pelo autor passam para as tabelas corretas (`characterSheets`, `locationSheets`, `itemSheets`, `factionSheets`, `creatureSheets`, `timelineEvents` ou `familyRelations`) e recebem estado `APPROVED`. O mesmo `fingerprint` não pode gerar uma nova aplicação.

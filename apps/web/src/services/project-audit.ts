...
export function auditProjectDump(dump: ProjectDump): AuditFinding[] {
  ...
  for (const candidate of tables.extractionCandidates ?? []) {
    if (!candidate.fingerprint || !candidate.baseFingerprint || !candidate.sourceHash || !candidate.runId || !candidate.payload || !candidate.heuristicVersion) findings.push({ area: 'Candidatos de extração', severity: 'erro', location: `extractionCandidates id=${candidate.id}`, message: 'Candidato sem chave versionada, fonte, execução, evidência ou versão da heurística.', suggestion: 'Não permitir a aplicação até completar metadados de proveniência; registros legados devem passar por remediação revisável.' });
    const minimumConfidence = 0.88;
    if (typeof candidate.confidence !== 'number' || candidate.confidence < minimumConfidence || candidate.confidence > 1) findings.push({ area: 'Candidatos de extração', severity: 'atenção', location: `extractionCandidates id=${candidate.id}`, message: `Confiança inválida ou abaixo da evidência mínima (${minimumConfidence}): ${candidate.confidence}.`, suggestion: 'Manter como pendente ou descartar; heurísticas legadas não podem ser promovidas novamente.' });

...
import { normalizeTemporalExpression } from '../../services/manuscript-extraction';
import './timeline.css';

const dateLabel = (date: string) => date || 'Sem data';

const chronologyFor = (event: TimelineEvent) => event.chronologicalSortKey ?? normalizeTemporalExpression(event.dateStr).sortKey;
const chronologicalCompare = (left: TimelineEvent, right: TimelineEvent) => {
  const leftKey = chronologyFor(left);
  const rightKey = chronologyFor(right);
  if (leftKey !== undefined && rightKey !== undefined) return leftKey - rightKey || left.sortOrder - right.sortOrder;
...
export default function TimelinePage() {
  ...
  const loadEvents = async () => {
    ...
    setEvents((await db.timelineEvents.where('timelineId').equals(activeId).toArray()).sort(chronologicalCompare));
  };
  ...
  const createEvent = async (event: FormEvent) => {
    ...
    const temporal = normalizeTemporalExpression(eventDate.trim());
    await db.timelineEvents.put({ id: crypto.randomUUID(), timelineId: activeTimeline.id, title: eventTitle.trim(), dateStr: eventDate.trim() || 'Sem data', description: eventDescription.trim() || undefined, chronologicalSortKey: temporal.sortKey, datePrecision: temporal.precision, temporalSource: 'MANUAL', calendarSystem: temporal.calendarSystem, era: temporal.era, year: temporal.year, month: temporal.month, day: temporal.day, sourceKind: 'MANUAL', sortOrder: Math.max(-1, ...events.map(item => item.sortOrder)) + 1, precursorEventIds: [], createdAt: new Date().toISOString() });
  ...
  const normalizeTimelineOrder = async () => {
    if (!activeTimeline) return;
    const stored = await db.timelineEvents.where('timelineId').equals(activeTimeline.id).toArray();
    const normalized = stored.map(event => {
      const temporal = normalizeTemporalExpression(event.dateStr);
      return { ...event, chronologicalSortKey: event.chronologicalSortKey ?? temporal.sortKey, datePrecision: event.datePrecision ?? temporal.precision, calendarSystem: event.calendarSystem ?? temporal.calendarSystem, era: event.era ?? temporal.era, year: event.year ?? temporal.year, month: event.month ?? temporal.month, day: event.day ?? temporal.day };
    }).sort(chronologicalCompare).map((event, sortOrder) => ({ ...event, sortOrder }));
    await db.timelineEvents.bulkPut(normalized);
    await loadEvents();
  };
  ...
  return <main className="page timeline-page">
    ...
    <section className="timeline-toolbar surface-card">
      ...
      {activeTimeline && <><button type="button" className="secondary-button" onClick={() => void normalizeTimelineOrder()}>Ordenar datas reconhecidas</button><button type="button" className="primary-button" onClick={() => setShowEventForm(value => !value)}>{showEventForm ? 'Cancelar' : 'Adicionar evento'}</button></>}
    </section>
